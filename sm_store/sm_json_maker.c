...
//#define NO_SPRINTF
//#define JSON_DEBUG_PADDING        // 2019.01.10 KSJ easy debug
#ifdef JSON_DEBUG_PADDING
int padding_cnt = 0;
#endif
...
#ifdef JSON_DEBUG_PADDING       // 2019.01.10 KSJ
    if ( dest[-1] == ',' || dest[-1] == '[' || dest[-1] == '{') dest = chtoa( dest, '\n' );
    for(int i = 0 ; i < padding_cnt ; i++)  dest = chtoa( dest, '\t' );
#endif
    if ( NULL == name )
        dest = chtoa( dest, '{' );
    else {
